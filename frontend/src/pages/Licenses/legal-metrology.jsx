...
const OverviewContent = () => (
  <section id="lm-overview-content" className="py-16 bg-slate-50/50 scroll-mt-24">
      ...
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <div className="space-y-6 text-slate-600 leading-relaxed italic">
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <span className="px-4 py-2 bg-white rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 shadow-sm uppercase italic">2009 Act Nexus</span>
            <span className="px-4 py-2 bg-white rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 shadow-sm uppercase italic">LMPC Compliance</span>
          </div>
        </div>
        <div className="bg-[#103B3E] p-8 rounded-[40px] text-white relative group overflow-hidden">
          ...
          <ul className="space-y-3 text-[10px] uppercase font-bold tracking-widest list-none">
            <li className="flex items-center gap-2"><CheckCircle size={14} className="text-[#C59B4E]" /> Net Quantity & MRP</li>
            <li className="flex items-center gap-2"><CheckCircle size={14} className="text-[#C59B4E]" /> MFG & Best Before Date</li>
            <li className="flex items-center gap-2"><CheckCircle size={14} className="text-[#C59B4E]" /> Importer/Packer Details</li>
            <li className="flex items-center gap-2"><CheckCircle size={14} className="text-[#C59B4E]" /> Consumer Care Helpline</li>
          </ul>
        </div>
      </div>
    </div>
...
const RulesProvisions = () => (
  <section id="lm-rules-provisions" className="py-16 bg-white scroll-mt-24">
    <div className="max-w-7xl mx-auto px-4">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
        {legalMetrologyRules.map((rule, i) => (
          <div key={i} className="flex flex-col p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white hover:shadow-xl transition-all h-full">
            ...
            <h6 className="font-bold text-slate-800 text-sm mb-2 italic uppercase tracking-tighter">{rule.title}</h6>
            <p className="text-[10px] text-slate-500 italic leading-relaxed">{rule.detail}</p>
          </div>
        ))}
      </div>
      <div className="p-8 bg-red-900 rounded-[32px] text-white flex flex-col md:flex-row gap-8 items-center shadow-2xl relative overflow-hidden">
        ...
        <div>
          ...
          <p className="text-xs text-slate-300 leading-relaxed italic">Importing measure-related items without registration can result in fines up to <strong>₹25,000</strong> and a maximum <strong>six-month prison sentence</strong> for subsequent offenses.</p>
        </div>
      </div>
    </div>
  </section>
...
const BenefitsContent = () => (
  <section id="lm-benefits-content" className="py-16 bg-slate-50/50 scroll-mt-24">
    <div className="max-w-7xl mx-auto px-4">
...
const EligibilityContent = () => (
  <section id="lm-eligibility-content" className="py-16 bg-white scroll-mt-24">
    <div className="max-w-7xl mx-auto px-4">
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        <div className="space-y-4">
          ...
          {lmpcEligibility.map((item, i) => (
            <div key={i} className="flex gap-4 items-center p-4 bg-slate-50 rounded-xl border-l-4 border-l-[#1A7F7D]">
              <CheckCircle size={16} className="text-[#1A7F7D]" />
              <span className="text-xs font-bold text-slate-700 italic uppercase">{item}</span>
            </div>
          ))}
        </div>
        <div className="p-8 bg-slate-100 rounded-[40px] border border-slate-200">
          ...
          <div className="space-y-4">
            {lmpcExemptions.map((item, i) => (
              <div key={i} className="flex gap-3 items-start bg-white p-4 rounded-[24px]">
                <AlertTriangle size={16} className="text-[#C19A5B] shrink-0" />
                <span className="text-[10px] text-slate-500 italic leading-relaxed">{item}</span>
              </div>
            ))}
          </div>
        </div>
...
const DocumentsContent = () => (
  <section id="lm-documents-content" className="py-16 bg-slate-50/50 scroll-mt-24">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {lmpcDocuments.map((doc, i) => (
          <div key={i} className="p-8 bg-white border border-slate-100 rounded-3xl flex flex-col items-center shadow-sm">
            ...
            <p className="text-[10px] text-slate-400 italic font-bold">{doc.detail}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="max-w-7xl mx-auto px-6">
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="order-1 lg:order-2 bg-[#103B3E] p-12 rounded-[50px] shadow-3xl text-white flex flex-col items-center text-center">
          ...
          <p className="text-slate-400 text-xs italic mb-8">"Providing world-class measurement compliance for India's leading manufacturers."</p>
          <button className="px-10 py-5 bg-[#C59B4E] text-white rounded-full font-extrabold uppercase tracking-widest text-[11px] hover:bg-[#a37d35] transition-all">Connect with expert</button>
        </div>
      </div>
    </div>
  </section>
    ...
    <div className="min-h-screen font-sans w-full overflow-x-hidden text-slate-900 selection:bg-[#1A7F7D] selection:text-white">
      ...
      <section className="relative w-full min-h-[auto] lg:min-h-screen flex items-center pt-32 pb-12 lg:pt-36 lg:pb-20">
        <div className="absolute inset-0 z-0">
          <img src={BackgroundImageSrc} alt="Legal Metrology Hero Background" className="w-full h-full object-cover" />
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
            <div className="w-full lg:w-1/2 flex flex-col items-start space-y-8">
              <div className="relative w-28 h-28 flex items-center justify-center">
                ...
                <div className="relative w-full h-full bg-[#1a1a1a] rounded-full border-2 border-[#C59B4E] flex flex-col items-center justify-center p-2">
                  ...
                  <span className="text-[#C59B4E] font-bold text-[10px] text-center leading-tight uppercase mt-1">Legal <br /> Metrology</span>
                  <span className="text-white text-[8px] uppercase mt-1 opacity-70">LMPC/2009 Act</span>
                </div>
              </div>
              <div className="space-y-4">
                <h1 className="text-3xl md:text-5xl lg:text-7xl font-extrabold text-white leading-[1] font-serif italic tracking-tighter">
                  Measurement <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E0F2F1] to-[#80CBC4] not-italic uppercase tracking-widest text-4xl md:text-6xl">Statutory Standards</span>
                </h1>
                <div className="space-y-3 pt-2">
                  {lmIntroBullets.map((bullet, i) => (
              ...
              <div className="flex items-center gap-6 py-2 overflow-x-auto no-scrollbar w-full">
                <div className="flex flex-col shrink-0">
                  <span className="text-white text-3xl font-black italic tracking-tighter uppercase underline decoration-[#C59B4E]">LMPC</span>
                  <span className="text-slate-400 text-[10px] uppercase tracking-widest font-bold">Certification</span>
                </div>
                <div className="h-10 w-[1px] bg-white/20"></div>
                <div className="flex flex-col shrink-0">
                  <span className="text-white text-3xl font-black italic tracking-tighter uppercase underline decoration-[#C59B4E]">2009 Act</span>
                  <span className="text-slate-400 text-[10px] uppercase tracking-widest font-bold">Compliance Nexus</span>
                </div>
              </div>
            </div>
            <div className="w-full max-w-sm lg:w-[400px]">
              <div className="bg-white rounded-[50px] shadow-3xl p-10 border border-white/5 relative overflow-hidden group">
                <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-[#C59B4E]/5 rounded-full blur-2xl group-hover:scale-150 transition-all"></div>
                <h2 className="text-2xl font-bold mb-1 text-center text-slate-800 tracking-tighter uppercase italic">Secure LMPC</h2>
                <p className="text-[11px] text-slate-400 mb-8 text-center uppercase tracking-widest font-bold">Professional Weight Compliance</p>
