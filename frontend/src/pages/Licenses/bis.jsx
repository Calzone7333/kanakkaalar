            ...
            <div className="grid lg:grid-cols-2 gap-8 items-center">
                <div className="space-y-8 text-slate-600 leading-relaxed italic">
                    ...
                    <div className="flex flex-wrap gap-5 pt-2">
                        <span className="px-4 py-2 bg-white rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 shadow-sm uppercase italic">BIS Act 2016</span>
                        <span className="px-4 py-2 bg-white rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 shadow-sm uppercase italic">ISI TRUST MARK</span>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-10">
                    {bisRoles.map((role, i) => (
                        <div key={i} className="p-8 bg-white border border-slate-100 rounded-[32px] shadow-sm flex flex-col items-center text-center">
                            <role.icon size={28} className="text-[#1A7F7D] mb-4" />
                            <h6 className="font-bold text-slate-800 text-sm uppercase italic mb-1 tracking-tighter">{role.title}</h6>
                            <p className="text-sm text-slate-400 leading-relaxed italic">{role.detail}</p>
                        </div>
                    ))}
                </div>
            </div>
    ...
    <section id="bis-types-content" className="py-12 md:py-16 bg-slate-50/50 scroll-mt-24">
        <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-10 mb-16">
                <div className="p-10 bg-slate-900 rounded-[48px] text-white flex gap-10 items-center group relative overflow-hidden">
                    <Scale size={48} className="text-[#C59B4E] shrink-0" />
                    <div>
                        <h5 className="font-bold text-lg uppercase italic tracking-widest text-[#C59B4E] mb-2">Mandatory Nexus</h5>
                        <p className="text-sm text-slate-400 italic font-medium leading-relaxed">Required for IT, Electronics, Steel and Cement sectors before any sale.</p>
                    </div>
                </div>
                <div className="p-10 bg-slate-900 rounded-[48px] text-white flex gap-10 items-center group relative overflow-hidden">
                    <Zap size={48} className="text-[#C59B4E] shrink-0" />
                    <div>
                        <h5 className="font-bold text-lg uppercase italic tracking-widest text-[#C59B4E] mb-2">Voluntary Hub</h5>
                        <p className="text-sm text-slate-400 italic font-medium leading-relaxed">Strategic certification for non-regulated sectors to build market value.</p>
                    </div>
                </div>
            </div>
            <div className="mb-4 text-center">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 max-w-6xl mx-auto">
                    {compulsoryProducts.map((p, i) => (
                        <div key={i} className="flex items-center gap-5 p-4 bg-white rounded-2xl border border-slate-100 italic shadow-sm">
                            <AlertTriangle size={16} className="text-red-500 shrink-0" />
                            <span className="text-sm font-bold text-slate-700 tracking-tight">{p}</span>
                        </div>
                    ))}
                </div>
            </div>
    ...
    <section id="bis-validity-content" className="py-12 md:py-16 bg-white scroll-mt-24">
        <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-8">
                {bisValidity.map((v, i) => (
                    <div key={i} className="flex gap-10 items-start p-8 bg-slate-50 rounded-[40px] group hover:bg-white hover:shadow-2xl transition-all">
                        ...
                        <div>
                            <h6 className="font-bold text-slate-800 text-lg italic mb-2 tracking-tight uppercase">{v.title}</h6>
                            <p className="text-sm text-slate-500 italic leading-relaxed">{v.detail}</p>
                        </div>
                    </div>
                ))}
            </div>
            <div className="mt-12 p-8 bg-blue-50 border-l-8 border-l-blue-600 rounded-[40px] shadow-sm flex items-center gap-10">
                ...
                <div>
                    <h5 className="font-extrabold text-blue-900 text-lg italic tracking-widest mb-1 uppercase">Surveillance Audit Protocol</h5>
                    <p className="text-sm text-blue-700 italic font-medium leading-relaxed">Regular checks of the manufacturing unit after certification to ensure any changes in production maintain the original ISI standard mark integrity.</p>
                </div>
            </div>
        </div>
    </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
            <div className="grid lg:grid-cols-2 gap-8 items-center">
                ...
                <div className="order-1 lg:order-2 bg-[#103B3E] p-12 rounded-[50px] shadow-3xl text-white flex flex-col items-center text-center">
                    ...
                    <p className="text-slate-400 text-sm italic mb-8 px-8">"Facilitating Bureau of Indian Standards compliance for world-class manufacturing houses."</p>
                        ...
                        <div className="w-full lg:w-3/5 space-y-8">
                            ...
                            <div className="flex gap-10 pt-2">
                                <div className="flex items-center gap-5 text-white/90 text-sm md:text-sm font-bold">
                                    <CheckCircle size={18} className="text-[#C59B4E]" /> ISI Trust Mark
                                </div>
                                <div className="flex items-center gap-5 text-white/90 text-sm md:text-sm font-bold">
                                    <Globe size={18} className="text-[#C59B4E]" /> Global Standards
                                </div>
                            </div>
                        </div>
                        <div className="w-full max-w-sm">
                            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-100">
                                <div className="p-8 md:p-8">
                                    <h3 className="text-xl font-bold text-slate-800 text-center mb-6">Apply BIS</h3>
