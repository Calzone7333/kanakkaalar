...
public class DealService {
    ...
    public DealService(DealRepository dealRepository, com.calzone.financial.order.OrderRepository orderRepository,
    ...
    public List<DealResponse> getAllDeals() {
        ...
        allDeals.sort((d1, d2) -> {
            if (d1.createdAt() == null || d2.createdAt() == null)
                return 0;
    ...
    public DealResponse updateDeal(Long id, java.util.Map<String, Object> updates) {
        ...
        if (updates.containsKey("name"))
            deal.setName((String) updates.get("name"));
        if (updates.containsKey("customer"))
            deal.setCustomer((String) updates.get("customer"));
        if (updates.containsKey("amount"))
            deal.setAmount((String) updates.get("amount"));
        if (updates.containsKey("stage"))
            deal.setStage((String) updates.get("stage"));
        if (updates.containsKey("probability"))
            deal.setProbability((Integer) updates.get("probability"));
        if (updates.containsKey("owner"))
            deal.setOwner((String) updates.get("owner"));
